...
/**
 * Checks if the element is an instance of the specified HTMLElement subclass
 * @param element Element to check.
 * @param expected Tag name(-s).
 * @returns `true` if the element is an instance of specified subclass(-es)
 * @example
 * ```tsx
 * is(<input />, "input");             // true
 * is(<input />, ["select", "input"]); // true
 * is(<input />, "div");               // false
 * ```
 */
export function is<T extends keyof ElementTagNameMap>(element: Node | null, expected: T | T[]): element is ElementTagNameMap[T] {
	...
	return Array.isArray(expected) ? expected.includes(tag as T) : tag === expected;
}
